...
) : IOnLoad
{
    ...
    private readonly List<MongoId> _holsterBaseClasses = [BaseClasses.PISTOL, BaseClasses.REVOLVER];
    ...
    public async Task OnLoadAsync(CancellationToken cancellationToken)
    {
        ...
        if (!NeedsRegeneration(mappingDir))
        {
            return;
        }
    ...
    private Task LoadVanillaItemsMapping()
    {
        ...
        >
        {
            ...
            {
                (id, _) =>
                    itemHelper.IsOfBaseclasses(id, _primaryBaseClasses)
                        && id != ItemTpl.GRENADELAUNCHER_FN40GL_02
                        && id != ItemTpl.GRENADELAUNCHER_FN40GL_03
                        && id != ItemTpl.SHOTGUN_MP43_12GA_SAWEDOFF_DOUBLEBARREL
                        && !itemHelper.GetItemName(id).Contains("RSP-30")
                        && !itemHelper.GetItemName(id).Contains("ROP-30")
                        && !itemHelper.GetItemName(id).Contains("ZiD SP")
                        && !itemHelper.GetItemName(id).Contains("NSV Utyos")
                        && !itemHelper.GetItemName(id).Contains("Master Hand")
            ...
            {
                (id, _) =>
                    itemHelper.IsOfBaseclass(id, BaseClasses.KNIFE)
                    && id != ItemTpl.KNIFE_SUPERFORS_DB_2020_DEAD_BLOW_HAMMER
                    && id != ItemTpl.KNIFE_INFECTIOUS_STRIKE
                    && id != ItemTpl.KNIFE_CHAINED_LABRYS,
                _scabbardMap
            },
            ...
            },
            {
                (id, item) =>
                    itemHelper.IsOfBaseclass(id, BaseClasses.AMMO)
                    && item.Properties?.Caliber == "Caliber23x75"
                    && id != ItemTpl.AMMO_23X75_CHER7M,
                _caliber23X75Map
            },
            {
                (id, item) =>
            ...
            },
            {
                (id, item) =>
                    itemHelper.IsOfBaseclass(id, BaseClasses.AMMO)
                    && item.Properties?.Caliber == "Caliber40x46"
                    && id != ItemTpl.AMMO_40X46_M716,
                _caliber40X46Map
            },
            {
                (id, item) =>
            ...
            },
            {
                (id, item) =>
                    itemHelper.IsOfBaseclass(id, BaseClasses.AMMO)
                    && item.Properties?.Caliber == "Caliber556x45NATO"
                    && id != ItemTpl.AMMO_556X45_6MM_BB,
            ...
            },
            {
                (id, item) =>
                    itemHelper.IsOfBaseclass(id, BaseClasses.AMMO)
                    && item.Properties?.Caliber == "Caliber9x18PM"
                    && !itemHelper.GetItemName(id).Contains("shrapnel"),
                _caliber9X18PmMap
            },
            {
                (id, item) =>
            ...
            },
            {
                (id, item) =>
                    ...
                    && item.Parent != BaseClasses.STIMULATOR
                    && item.Parent != BaseClasses.DRUGS
                    && id != BaseClasses.MEDS
                    && id != BaseClasses.STIMULATOR
                    && id != ItemTpl.MEDKIT_SANITARS_FIRST_AID_KIT
    ...
    private Task LoadVanillaAppearanceMapping()
    {
        ...
        foreach (var (id, item) in allItems)
        {
            if (item.Properties.BodyPart is null)
            {
                continue;
            }
            if (item.Properties.BearTemplateId is not null)
            {
                continue;
            }
            if (item.Properties.UsecTemplateId is not null)
            {
                continue;
            }
            ...
            if (!allowedParents.Contains(item.Parent))
            {
                continue;
            }
