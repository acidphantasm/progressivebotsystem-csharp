...
) : IOnLoad
{
    private readonly Dictionary<string, string[]> _expectedPresetStructure = new()
    {
        ...
        },
        {
            "Appearance",
            [
                "Tier1_appearance.json",
                "Tier2_appearance.json",
                "Tier3_appearance.json",
                "Tier4_appearance.json",
                "Tier5_appearance.json",
                "Tier6_appearance.json",
                "Tier7_appearance.json",
            ]
        },
        {
            "Chances",
        ...
        },
        {
            "Equipment",
            [
                "Tier1_equipment.json",
                "Tier2_equipment.json",
                "Tier3_equipment.json",
                "Tier4_equipment.json",
                "Tier5_equipment.json",
                "Tier6_equipment.json",
                "Tier7_equipment.json",
            ]
        },
        {
            "Mods",
    ...
    public async Task OnLoadAsync(CancellationToken cancellationToken)
    {
        ...
        {
            await AssignJsonDataFromPreset(pathToMod);
        }
        else
        {
    ...
    public async Task AssignJsonData(string pathToMod)
    {
        ...
        var tierNumbers = JsonFileHashes
            .Hashes.Keys.Where(k => k.StartsWith("Ammo/Tier", StringComparison.OrdinalIgnoreCase))
    ...
    private bool ValidatePresetFolder(string fullPathToPresetRoot, out string errorMessage)
    {
        ...
        foreach (var kvp in _expectedPresetStructure)
        {
            ...
            var filesInFolder = Directory.GetFiles(folderPath).Select(Path.GetFileName).ToArray();
    ...
    {
        var fullPathToPreset = Path.Combine(pathToMod, "Presets", ModConfig.Config.PresetName);
        if (!Directory.Exists(fullPathToPreset))
        {
            return false;
        }
        if (!ValidatePresetFolder(fullPathToPreset, out var errorMessage))
