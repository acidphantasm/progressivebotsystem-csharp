    ...
    : IOnLoad
{
    ...
    private int CheckChadOrChill(int tierNumber)
    {
        ...
        {
            return ModConfig.Config.GeneralConfig.TarkovAndChill ? randomUtil.GetInt(1, 7) : 7;
        }
    ...
    )
    {
        if (!ignoreCheck)
        {
            tierNumber = CheckChadOrChill(tierNumber);
        }
        ...
        switch (botRole)
        {
            ...
            case "assault":
                if (
                    ...
                    || ModConfig.Config.GeneralConfig.OnlyChads
                    || ModConfig.Config.ScavBots.AdditionalOptions.EnableScavAttachmentTiering
                )
                {
    ...
    )
    {
        ...
        {
            "pmcusec" => tieredEquipmentData.PmcUsec.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "pmcbear" => tieredEquipmentData.PmcBear.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            ...
            "bossboarsniper" => tieredEquipmentData.BossBoarSniper.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bossbully" => tieredEquipmentData.BossBully.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bossgluhar" => tieredEquipmentData.BossGluhar.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bosskilla" => tieredEquipmentData.BossKilla.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bosskillaagro" => tieredEquipmentData.BossKillaAgro.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bosskojaniy" => tieredEquipmentData.BossKojaniy.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bosskolontay" => tieredEquipmentData.BossKolontay.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bosssanitar" => tieredEquipmentData.BossSanitar.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bosstagilla" => tieredEquipmentData.BossTagilla.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bosstagillaagro" => tieredEquipmentData.BossTagillaAgro.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bosspartisan" => tieredEquipmentData.BossPartisan.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bosszryachiy" => tieredEquipmentData.BossZryachiy.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "bossknight" => tieredEquipmentData.BossKnight.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "followerbigpipe" => tieredEquipmentData.FollowerBigPipe.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "followerbirdeye" => tieredEquipmentData.FollowerBirdeye.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "sectantpriest" => tieredEquipmentData.SectantPriest.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
            "sectantwarrior" => tieredEquipmentData.SectantWarrior.Equipment.ToDictionary(
                kvp => kvp.Key,
                kvp => new Dictionary<MongoId, double>(kvp.Value)
            ),
    ...
    private int NewTierCalc(int tierNumber, int minTier, int maxTier)
    {
        ...
        var newTier =
            Math.Floor(random.NextDouble() * (maxTier - minTier + 1) + minTier) >= tierNumber
                ? tierNumber - 1
