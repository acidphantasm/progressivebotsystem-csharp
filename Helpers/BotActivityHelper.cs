...
public class BotActivityHelper(ApbsLogger apbsLogger) : IOnLoad
{
    ...
    public bool IsBotEnabled(string botType)
    {
        ...
        if (botType is "usec" or "bear" or "pmc")
        {
            botType = "pmcusec";
        }
    ...
    private bool DoesBotExist(string botType)
    {
        ...
        return _bosses.Contains(botType)
            || _followers.Contains(botType)
            || _pmcs.Contains(botType)
            || _scavs.Contains(botType)
            || _specials.Contains(botType)
            || _events.Contains(botType);
    }
    ...
    private bool IsBotDisabled(string botType)
    {
        ...
        if (_alwaysDisabled.Contains(botType))
        {
            return true;
        }
        if (_events.Contains(botType))
        {
            return true;
        }
        if (_bosses.Contains(botType))
        {
            return !ModConfig.Config.BossBots.Enable;
        }
        if (_followers.Contains(botType))
        {
            return !ModConfig.Config.FollowerBots.Enable;
        }
        if (_pmcs.Contains(botType))
        {
            return !ModConfig.Config.PmcBots.Enable;
        }
        if (_scavs.Contains(botType))
        {
            return !ModConfig.Config.ScavBots.Enable;
        }
        if (_specials.Contains(botType))
        {
            return !ModConfig.Config.SpecialBots.Enable;
        }
