...
public class InventoryMagGenHelper
{
    ...
    public List<Item> CreateMagazineWithAmmo(
        MongoId magazineTpl,
        MongoId ammoTpl,
        Dictionary<string, Dictionary<MongoId, double>> ammoPool,
        string ammoCaliber,
    ...
    private void FillMagazineWithCartridge(
    ...
    )
    {
        ...
        var desiredMagazineTpls = modPool
            .Where(magTpl =>
            {
                ...
                return maxCount is not null && maxCount < max && maxCount >= min;
            })
            .ToList();
        ...
        {
            _apbsLogger.Debug(
                $"[MAGAZINE] Magazine size filter for: {weaponTpl} does not have small capacity magazines available in tier {tier}. Using any available magazine."
            );
    ...
    public TemplateItem GetFallbackFittingMagazine(
