...
public class GenerateInventoryPatch : AbstractPatch
{
    ...
    )
    {
        if (
            ...
            || !_botActivityHelper.IsBotEnabled(botGenerationDetails.Role)
        )
        {
            return true;
        }
        ...
        if (
            ...
            && !shouldQuest
            && ModConfig.Config.PmcBots.PovertyConfig.Enable
            && !ModConfig.Config.GeneralConfig.BlickyMode
            && tierNumber > 1
            && _randomUtil.GetChance100(ModConfig.Config.PmcBots.PovertyConfig.Chance)
        )
        {
            ...
            _apbsLogger.Debug(
                $"[POVERTY] Level{botGenerationDetails.BotLevel} PMC was flagged to be in poverty. Old Tier: {tierNumber}, New Tier: {newTierNumber}"
            );
        ...
        _customBotInventoryGenerator.GenerateAndAddEquipmentToBot(
            botId,
            sessionId,
            templateInventory,
            chances,
        ...
        _customBotInventoryGenerator.GenerateAndAddWeaponsToBot(
            botId,
            templateInventory,
            chances,
            sessionId,
        ...
        _customBotLootGenerator.GenerateLoot(
            botId,
            sessionId,
            botJsonTemplate,
            botGenerationDetails,
