...
public class GenerateBotLevel : AbstractPatch
{
    ...
    )
    {
        ...
        var fractionalExp =
            level < maxLevelIndex ? _randomUtil.GetInt(0, expTable[level].Experience - 1) : 0;
    ...
    )
    {
        ...
        var minPossibleLevel = levelOverride is not null
            ? Math.Min(Math.Max(levelDetails.Min, levelOverride.Min), maxAvailableLevel)
        ...
        if (
            ...
            && (
                botGenerationDetails.Role.Contains("assault")
        ...
        if (
            ...
            && ModConfig.Config.PmcBots.AdditionalOptions.EnablePrestigeAnyLevel
            && RaidInformation.HighestPrestigeLevel != 0
        )
        {
            maxLevel = 79;
            minLevel = 1;
    ...
    private static int SetBotPrestigeInfo(int level, BotGenerationDetails botGenerationDetails)
    {
        ...
        var botCanPrestige =
            playerLevel >= minPlayerLevelForBotsToPrestige
            || playerLevel >= level - 15 && isPlayerPrestiged
            || isPlayerPrestiged;
        ...
        if (botCanPrestige)
        {
            ...
            {
                botPrestigeLevel =
                    playerPrestigeLevel >= maxPrestige
                        ? _randomUtil.GetInt(0, maxPrestige)
                        : _randomUtil.GetInt(0, playerPrestigeLevel);
            ...
            if (level <= 20 - Math.Abs(playerLevel - 79))
            {
                botPrestigeLevel =
                    playerPrestigeLevel >= maxPrestige
                        ...
                        : _randomUtil.GetInt(
                            botPrestigeLevel,
                            Math.Min(playerPrestigeLevel + 1, 4)
                        );
                hasBotTriedAlready = true;
            }
